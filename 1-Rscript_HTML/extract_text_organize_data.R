...
# script 1: search coordinates in HTML files 
...
# Load functions 
source("1-Rscript_HTML/retLongLatFunction.R")

# load data with dois
csv_with_doi <- read.csv(here ("data","Basic_Data", "papers_doi.csv"),sep=",")

# load data with DOI for is.na
csv_doi_NA <- read.csv(here ("data", "Basic_Data", "get_missing_dois.csv"),sep=";")
...
doi_to_get <- iconv(doi_to_get, from = "ISO-8859-1", to = "UTF-8")
...
# save - you can find this result in the  folder "1-processed_data_HTML_coord_extraction"
save(text_to_save,file=here ("1-processed_data_HTML","saved_text.RData"))
# load extracted text (if you're not in your institution)
load(here ("1-processed_data_HTML","saved_text.RData"))
...
csv_used_papers <- read.csv(here ("data", "Basic_Data","papers_list_used_31_03_21.csv"),
                            encoding = "latin")

# try to find a common ID to filtering
csv_used_papers [grep ("BARBOSA-FILHO", 
                       csv_used_papers$PAPER_ID),"PAPER_ID"] 

# replace any "," by nothing
csv_used_papers$SR <- gsub (",","",csv_used_papers$SR)
csv_used_papers$SR <- gsub (":","",csv_used_papers$SR)
csv_used_papers$PAPER_ID <- gsub (",","",csv_used_papers$PAPER_ID)
...
res_topic_modeling <- read.csv (here ("data","Basic_Data", "Planilha_paper_x_topicos.csv"))
...
res_topic_modeling <- res_topic_modeling [which(res_topic_modeling$Paper_ID %in% csv_used_papers$PAPER_ID),]
# now the used papers
csv_used_papers <- csv_used_papers [which(csv_used_papers$PAPER_ID %in%  res_topic_modeling$Paper_ID),]

# cf
# re_topic_modeling & csv_used papers are all equal...
table(res_topic_modeling$Paper_ID == csv_used_papers$PAPER_ID)
...
#
## go to excel
#write.csv(tab_inconsistencies,file=here("data","Basic_Data","inconsistencies.csv"))
...
table(is.na(csv_with_doi$DI [match(csv_used_papers$PAPER_ID,gsub(",","",csv_with_doi$X))]))
...
# iconv
csv_used_papers$used_dois <- iconv(csv_used_papers$used_dois, from = "ISO-8859-1", to = "UTF-8")

#used_dois <- csv_with_doi$DI [which(csv_with_doi$X %in% csv_used_papers$PAPER_ID)]
...
# save
save(resu_dxdoi,file=here ("1-processed_data_HTML","resu_dxdoi.RData"))
load(here ("1-processed_data_HTML","resu_dxdoi.RData"))
...
save (findtLongLat_dxdoi,
      file = here ("1-processed_data_HTML", "findtLongLat_dxdoi.RData"))

# load these latlong coords
load(here ("1-processed_data_HTML","findtLongLat_dxdoi.RData"))
...
save (matched_data_coordinates_mpas,
	file=  here ("1-processed_data_HTML", "spatial_data_topic_modeling.RData"))
# also in csv
write.csv (matched_data_coordinates_mpas,
	file= here ("1-processed_data_HTML", "spatial_data_topic_modeling.csv"))
